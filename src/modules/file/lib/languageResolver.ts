...
/**
 * Filename → CodeMirror language extension. Each loader is a dynamic
 * import so language packs only enter the bundle when a matching file is
 * opened. Loaders return either a ready Extension (lang-* packages) or a
 * raw StreamParser (legacy-modes); the latter is wrapped in StreamLanguage
 * before returning.
 */
...
const loaders: Record<string, LanguageLoader> = {
  ...
  // Legacy-modes return raw StreamParser → wrapped below
